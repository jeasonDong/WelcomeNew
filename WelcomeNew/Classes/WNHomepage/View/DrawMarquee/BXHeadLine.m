...
#import "BXHeadLine.h"
#import "BXColorGradientView.h"
#define KBXHeadLineMargin 10
typedef NS_ENUM(NSInteger, BXMarqueeTapMode) {
    BXMarqueeTapForMove = 1,
    BXMarqueeTapForAction = 2
};
@interface BXHeadLine ()
...
@property (nonatomic,strong) BXColorGradientView  *viewTop;
@property (nonatomic,strong) BXColorGradientView  *viewBottom;
@property (nonatomic,strong) UIButton             *bgBtn;
@property (nonatomic,copy  ) actionBlock           tapAction;
@property (nonatomic,assign) BXMarqueeTapMode      tapMode;
@end
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        UILabel *label1          = [[UILabel alloc]initWithFrame:CGRectMake(KBXHeadLineMargin, 0, frame.size.width, _h)];
        UILabel *label2          = [[UILabel alloc]initWithFrame:CGRectMake(KBXHeadLineMargin, _h, frame.size.width, _h)];
...
- (void)setHasGradient:(BOOL)hasGradient{
    ...
    if (hasGradient) {
        _viewTop = [BXColorGradientView createWithColor:self.bgColor frame:CGRectMake(0, 0, _w, _h * 0.2) direction:BXGradientToBottom];
        _viewBottom = [BXColorGradientView createWithColor:self.bgColor frame:CGRectMake(0, _h * 0.8, _w, _h * 0.2) direction:BXGradientToTop];
...
- (void)changeTapMarqueeAction:(actionBlock)action{
    ...
    self.tapMode = BXMarqueeTapForAction;
}
- (UIButton *)bgBtn
{
    if (!_bgBtn) {
